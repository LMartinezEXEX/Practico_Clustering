...
def get_word_frequency(words):
    ...
    words_freq = {}
    counter = Counter([word.lemma_.lower() for word in words])
    for word in counter:
...
def get_words_vectors(doc, tokens, words_freq):
    ...
    for token in tokens:
        ...
        for pos in range(start, end):
            co_token = doc[pos]
            if co_token.is_alpha and (co_token.lemma_.lower() in words_freq and words_freq[co_token.lemma_.lower()] >= MIN_FREQUENCY):
                if not co_token.lemma_.lower() in features:
                    features[co_token.lemma_.lower()] = 0
                features[co_token.lemma_.lower()] += 1
    ...
    for word in word_dicc:
        # Remove vectors with only zeros
        if len(word_dicc[word]) > 0:
            word_ids[word] = wid
...
def save_clusters(dicc):
    ...
    i = 0
    with open('Resultados/iteracion_2_CLUSTERS.txt', 'w') as file:
        for index in dicc:
...
if __name__ == "__main__":
    ...
    tsne_matrix = tsne(co_matrix)

    lsa_matrix = LSA(co_matrix)
